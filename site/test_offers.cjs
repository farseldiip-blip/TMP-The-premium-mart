...
let EDGE_PATH = edgePaths[0];
if (!fs.existsSync(EDGE_PATH)) {
  console.log('ERROR: Edge not found');
  process.exit(1);
}
console.log('Edge found at:', EDGE_PATH);
...
async function testViewport(vp) {
  ...
  await page.goto('http://127.0.0.1:5173/offers.html', { waitUntil: 'networkidle0', timeout: 15000 });
  await sleep(1000);
  ...
  const result = await page.evaluate(() => {
    ...
    return {
      gridHtml: grid ? grid.innerHTML.length : 0,
      emptyVisible: empty ? empty.style.display !== 'none' : false,
  ...
  await page.screenshot({ path: `pupp_offers_${vp.label}.png`, fullPage: true });
  await browser.close();
  
  console.log(`[${vp.label}] cards=${result.cardCount}, overflow=${result.horizontalOverflow}, errors=${errors.length}`);
...
async function main() {
  ...
  for (const vp of viewports) {
    try {
      ...
      results.push({ ...vp, errors: [e.message], cardCount: -1 });
    }
  }
  ...
  for (const r of results) {
    ...
    console.log(`${r.label}: ${status} (${r.cardCount} cards, overflow=${r.horizontalOverflow})`);
  }
